...
class Motor:
    ...
    def obs(self):
        line = self.s.recv(256).decode() # todo: work with bytes no need to convert
        line  = line.split(',')
        if len(line)>6:
    ...
    def log(self):
        print('currents> id=>{} A | iq=>{} A\nvoltages> ud=> {} V | uq=>{} V\nmotor> w_r => {}rad/sec | theta => {} degree'.format(self.id,self.iq, self.ud,self.uq,self.wr, self.pos))
